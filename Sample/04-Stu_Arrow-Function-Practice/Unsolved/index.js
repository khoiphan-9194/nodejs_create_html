...
function doubledFunction(element){
  return element * 2;
}

let doubleed = map(numbers, doubledFunction);
...
var filter = (arr, cb) => {
  ...
  for (var index = 0; index < arr.length; index++) {
    var currentElement = arr[index];
    if (cb(currentElement, index))//if the current element is true 
      {
...
function evenNumbersFunction(currentElement){
  return currentElement % 2 === 0;
}

let evenNumberess = filter(numbers, evenNumbersFunction);
...
var netflixQueue = {
  ...
  watchMovie: function () {this.queue.pop()},
